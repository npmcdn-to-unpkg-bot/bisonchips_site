...
def export_event(event, format='ical'):
    ...
    uid = hashlib.sha1(event_string.encode('utf-8')).hexdigest() + '@bisonchips'
    ...
    ical_components = [
        ...
        'DTSTART;TZID=America/New_York:' + start_datetime.strftime('%Y%m%dT%H%M%S'),
        'DTEND;TZID=America/New_York:' + end_datetime.strftime('%Y%m%dT%H%M%S'),
        'END:VEVENT',
        'END:VCALENDAR'
    ]
